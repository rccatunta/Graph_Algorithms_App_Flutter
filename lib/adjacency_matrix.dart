...
class AdjacencyMatrix extends StatelessWidget{
   List<int> edge1,edge2;
   List<String> nameNode,nameEdge;
  int size;
  AdjacencyMatrix(this.edge1,this.edge2,this.size,this.nameNode,this.nameEdge);
  @override
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        body: _AdjacencyMatrix()
    );
  }
  Widget _AdjacencyMatrix(){
    int gridLength = size+1;
    print("Grid Length "+gridLength.toString());
    return Column(
      children: <Widget>[
        AspectRatio(
          aspectRatio: 1.0,
  ...
  Widget _buildGridItems(BuildContext context,int index){
    ...
    return GridTile(
      child: Container(
        ...
        child: Center(
          child: Text(conn,style: TextStyle(color: color),
          ),
        ),
      ),
    );
